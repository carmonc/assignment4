...
class TspProblem(SearchProblem):
    ...
    def _tour_length(self, s):
        ...
        tourlen = 0
        hops = len(s)
        for item in range(0,hops-1):
            lhs_idx = s[item]
            rhs_idx = s[item+1]
            #tourlen += self.distances[int(lhs_idx)][int(rhs_idx)]
            tourlen = tourlen + self.distances[int(lhs_idx)][int(rhs_idx)]
        return tourlen 
...
problem = TspProblem(matrix, [0, 4, 3, 2, 1, 8, 7, 6, 5, 11, 10, 9, 0])
#problem = TspProblem(matrix , sys.argv[2:])
